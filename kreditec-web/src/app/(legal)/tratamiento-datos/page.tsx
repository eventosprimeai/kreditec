...
import { Key, Scale, User, Target, Clock, Shield, Lock, FileText, Database, Check, Mail, Phone } from 'lucide-react';
...
const sections = [
  {
    ...
    Icon: Key,
    content: 'KREDITEC solamente comparte tus datos personales a terceros con los que esté contractual o legalmente obligado a facilitarlos, así como a:',
    items: [
  ...
  },
  {
    ...
    title: 'Tus Derechos',
    Icon: User,
    rights: ['Acceso', 'Eliminación', 'Rectificación', 'Actualización', 'Oposición', 'Suspensión', 'Portabilidad', 'Revocación del consentimiento'],
  },
  {
    id: 'finalidad',
    ...
    Icon: Target,
    finalidades: [
      { title: 'Prestación de servicios y productos contratados', desc: 'Incluye la gestión de las relaciones precontractuales, la ejecución de los contratos de servicio y el soporte a través de nuestro personal de ventas, canales oficiales, sitio web y oficinas.' },
      { title: 'Envío de comunicaciones informativas y comerciales', desc: 'Para remitir información y material sobre nuestros productos y servicios a través de cualquier medio. El Titular puede retirar su consentimiento para recibir campañas de mercadotecnia directa en cualquier momento.' },
      { title: 'Atención de consultas o reclamos', desc: 'Para gestionar, responder y dar seguimiento a todas tus consultas, peticiones, quejas o reclamos de manera eficiente.' },
      { title: 'Información sobre cambios en las condiciones', desc: 'Para comunicarte oportunamente sobre cualquier actualización o modificación en las condiciones, términos y características de los productos y servicios que has contratado.' },
      { title: 'Cumplimiento de obligaciones legales', desc: 'Para cumplir con todas las obligaciones legales, fiscales y regulatorias a las que KREDITEC se encuentra sujeto ante las autoridades competentes.' },
    ],
  },
  {
    id: 'conservacion',
  ...
  },
  {
    ...
    title: 'Protección',
    Icon: Shield,
    text: 'Desde KREDITEC hemos implementado una serie de medidas legales, técnicas y organizativas para que los datos personales se encuentren protegidos de accesos no autorizados, pérdidas, alteraciones o divulgaciones indebidas.',
  },
];
    ...
    <main className="min-h-screen bg-white dark:bg-[#060f09]">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="space-y-6">
          {sections.map((s) => (
            <section key={s.id} id={s.id} className="scroll-mt-24 p-6 bg-gray-50 dark:bg-[#0d1f13] rounded-2xl border border-gray-200 dark:border-[#1a3a22]">
              <div className="flex items-center gap-3 mb-5">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#00bc4c] to-[#009940] flex items-center justify-center flex-shrink-0">
                  <s.Icon size={18} className="text-white" />
                </div>
                <h2 className="text-xl font-bold text-[#002d14] dark:text-white">{s.title}</h2>
              </div>

              {s.content && <p className="text-gray-700 dark:text-gray-300 text-[15px] leading-relaxed mb-4">{s.content}</p>}
              {s.text && <p className="text-gray-700 dark:text-gray-300 text-[15px] leading-relaxed">{s.text}</p>}

              {s.items && (
                <div className="space-y-2">
                  {s.items.map((item, j) => (
                    <div key={j} className="flex gap-3 items-start p-3 bg-white dark:bg-[#060f09] rounded-lg border border-gray-100 dark:border-[#1a3a22]">
              ...
              {s.rights && (
                <>
                  <p className="text-gray-700 dark:text-gray-300 text-[15px] leading-relaxed mb-4">Los Titulares de datos personales pueden ejercitar sus derechos de:</p>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {s.rights.map((right, j) => (
                      <div key={j} className="flex items-center gap-2 p-3 bg-white dark:bg-[#060f09] rounded-lg border border-gray-100 dark:border-[#1a3a22]">
                        <span className="w-4 h-4 rounded-full bg-[#00bc4c] flex-shrink-0" />
                        <span className="text-gray-700 dark:text-gray-300 text-sm font-medium">{right}</span>
                      </div>
                    ))}
                  </div>
                  <p className="text-gray-600 dark:text-gray-400 text-sm mt-4 leading-relaxed">Para ejercer cualquiera de estos derechos, dirija su solicitud al correo electrónico indicado al final de este documento.</p>
                </>
              )}

              {s.finalidades && (
                <div className="space-y-3">
                  {s.finalidades.map((f, j) => (
                    <div key={j} className="flex gap-3 items-start p-4 bg-white dark:bg-[#060f09] rounded-xl border border-gray-100 dark:border-[#1a3a22]">
                      <Check size={14} className="text-[#00bc4c] flex-shrink-0 mt-0.5" />
                      <div>
                        <p className="font-semibold text-[#002d14] dark:text-white text-sm">{f.title}</p>
                        <p className="text-gray-600 dark:text-gray-400 text-sm mt-1 leading-relaxed">{f.desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="mt-12 p-6 bg-[#002d14] dark:bg-[#001a0c] rounded-2xl">
          ...
          <a href="mailto:protecciondedatos@kreditecsa.com" className="inline-flex items-center gap-2 bg-[#00bc4c] text-white px-5 py-2.5 rounded-xl font-semibold text-sm hover:bg-[#00e05a] transition-colors">
            <Mail size={16} /> protecciondedatos@kreditecsa.com
          </a>
        </div>
