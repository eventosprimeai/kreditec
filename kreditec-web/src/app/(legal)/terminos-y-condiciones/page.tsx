...
import { Lock, Settings, Shield, FileText, Check, Building, Phone, Mail } from 'lucide-react';
...
const sections = [
  { id: 'definiciones', title: 'Definiciones', paras: [
  ...
  { id: 'objeto-ambito', title: 'Objeto y Ámbito de Aplicación', paras: [
    ...
    'KREDITEC podrá modificar, actualizar o eliminar los contenidos y funcionalidades del Sitio Web, y modificar estos Términos y Condiciones de Uso cuando lo estime conveniente para su adecuada funcionalidad y en beneficio de KREDITEC, sin necesidad de previo aviso.',
  ]},
  { id: 'propiedad-intelectual', title: 'Propiedad Intelectual', paras: [
    'El Usuario y/o Cliente se obliga a respetar los derechos de propiedad intelectual de KREDITEC, y de terceros.',
    'El Usuario y/o Cliente podrá descargarse del Sitio Web en su terminal siempre que sea para uso privado, sin ningún fin comercial, por lo que no podrá explotar, reproducir, distribuir, modificar, comunicar públicamente, ceder, transformar o usar el contenido de la web con fines públicos o comerciales.',
    'KREDITEC es titular del software, derechos de autor y demás elementos de propiedad intelectual que se encuentren involucrados en el Sitio Web. No es permitido la reproducción de estos con fines comerciales y sin la autorización expresa de KREDITEC.',
  ...
  { id: 'responsabilidad', title: 'Responsabilidad de KREDITEC', paras: [
  ...
  { id: 'disponibilidad', title: 'Disponibilidad del Sitio Web', paras: [
  ...
  { id: 'enlaces-terceros', title: 'Enlaces de Terceros', paras: [
  ...
  { id: 'proteccion-datos', title: 'Protección de Datos Personales', paras: [
  ...
  { id: 'cookies', title: 'Uso de Cookies', paras: [
  ...
  { id: 'legislacion', title: 'Legislación Aplicable', paras: [
    'Las presentes Condiciones de Uso se rigen por las leyes de la República del Ecuador.',
  ]},
  { id: 'modificaciones', title: 'Modificaciones de las Condiciones de Uso', paras: [
  ...
  { id: 'conflictos', title: 'Solución de Conflictos', paras: [
...
const relatedLinks = [
  { href: '/politica-de-privacidad', icon: <Lock size={18} className="text-[#00bc4c]" />, label: 'Política de Privacidad' },
  { href: '/politica-de-cookies', icon: <Settings size={18} className="text-[#00bc4c]" />, label: 'Política de Cookies' },
  { href: '/politica-general-de-datos', icon: <Shield size={18} className="text-[#00bc4c]" />, label: 'Política General de Datos' },
];
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="bg-gradient-to-br from-[#00bc4c]/5 to-[#002d14]/5 dark:from-[#00bc4c]/10 dark:to-[#001a0c] border border-[#00bc4c]/20 rounded-2xl p-6 mb-12">
          ...
          <div className="mt-4 pt-4 border-t border-[#00bc4c]/20 grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
            <div>
              <span className="text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wide flex items-center gap-1.5"><Building size={11} /> Empresa</span>
              <p className="text-gray-800 dark:text-gray-200 font-medium mt-1">KREDITECONK S.A.S.</p>
            </div>
            <div>
              <span className="text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wide flex items-center gap-1.5"><Phone size={11} /> Teléfono</span>
              <p className="text-gray-800 dark:text-gray-200 font-medium mt-1">+593 96 341 3419</p>
            </div>
            <div>
              <span className="text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wide flex items-center gap-1.5"><Mail size={11} /> Correo legal</span>
              <p className="text-gray-800 dark:text-gray-200 font-medium mt-1">legal@kreditecsa.com</p>
            </div>
          </div>
        </div>
            ...
            <section key={section.id} id={section.id} className="scroll-mt-24">
              ...
              <div className="ml-12 space-y-3">
                {section.paras.map((para, j) => {
                  ...
                  if (isFullBold) return <p key={j} className="font-bold text-[#002d14] dark:text-[#00bc4c]">{para.replace(/\*\*/g, '')}</p>;
                  if (isBold) {
                    const [label, ...rest] = para.split(':** ');
                    return <p key={j} className="text-gray-700 dark:text-gray-300 leading-relaxed text-[15px]"><strong className="text-[#002d14] dark:text-white">{label.replace('**', '')}:</strong> {rest.join(': ')}</p>;
                  }
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4">
          {relatedLinks.map((l) => (
            <Link key={l.href} href={l.href} className="group flex items-center gap-3 p-4 bg-gray-50 dark:bg-[#0d1f13] border border-gray-200 dark:border-[#1a3a22] rounded-xl hover:border-[#00bc4c]/50 transition-all">
              {l.icon}
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400">Ver también</p>
                <p className="text-sm font-semibold text-gray-800 dark:text-gray-200 group-hover:text-[#00bc4c] transition-colors">{l.label}</p>
              </div>
            </Link>
          ))}
        </div>
